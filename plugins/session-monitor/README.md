...
├── swiftbar/
│   └── claude-sessions.5s.sh            # SwiftBar プラグイン (5 秒更新)
└── README.md
```
...
SwiftBar のプラグインディレクトリ（デフォルト: `~/.config/swiftbar` or `~/Library/Application Support/SwiftBar`）にスクリプトを symlink で置く:

```bash
# 例: ~/.config/swiftbar を使っている場合
ln -s "$(realpath plugins/session-monitor/swiftbar/claude-sessions.5s.sh)" \
      "$HOME/.config/swiftbar/claude-sessions.5s.sh"
```
