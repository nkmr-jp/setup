...
# Write cwd for external scripts (e.g. Smart Selection handlers)
# Per-session file keyed by session_id, short ID embedded in statusline text
if [ -n "$cwd" ] && [ -n "$session_id" ]; then
  echo "$cwd" > "/tmp/claude-statusline-cwd-${session_id}"
  ln -sf "claude-statusline-cwd-${session_id}" /tmp/claude-statusline-cwd
fi
# transcript_path
...
build_prompt_component() {
  ...
  if [ -n "$prompt" ]; then
    ...
    # Cap maximum length at 120 characters for readability
    if [ $max_length -gt 120 ]; then
      max_length=120
    fi
...
build_link_component() {
  ...
  if is_iterm2 && [ -n "$session_id" ]; then
    printf "Edit Diff"
  else
    local issue_identifier
...
# Build limits component (context window + rate limits combined)
# Normal: "(6%/6%/3%)" (ctx/5h/7d)
# When any ≥50%, show reset time for that item: "(6%/62%~2h30m/3%)"
build_limits_component() {
  local input_json="$1"

  local ctx_pct five_hour_pct seven_day_pct five_hour_resets seven_day_resets
  ctx_pct=$(echo "$input_json" | jq -r '.context_window.used_percentage // empty')
  five_hour_pct=$(echo "$input_json" | jq -r '.rate_limits.five_hour.used_percentage // empty')
  seven_day_pct=$(echo "$input_json" | jq -r '.rate_limits.seven_day.used_percentage // empty')
  five_hour_resets=$(echo "$input_json" | jq -r '.rate_limits.five_hour.resets_at // empty')
  seven_day_resets=$(echo "$input_json" | jq -r '.rate_limits.seven_day.resets_at // empty')

  if [ -z "$ctx_pct" ] && [ -z "$five_hour_pct" ] && [ -z "$seven_day_pct" ]; then
    return
  fi

  # Color based on usage percentage
  local color_ctx color_5h color_7d
  for var_name in ctx_pct five_hour_pct seven_day_pct; do
    eval "local val=\$$var_name"
    [ -z "$val" ] && val=0
    val=$(printf "%.0f" "$val")
    local c="$COLOR_GRAY"
    [ "$val" -ge 50 ] && c="$COLOR_ORANGE"
    [ "$val" -ge 80 ] && c="$COLOR_RED"
    case "$var_name" in
      ctx_pct) color_ctx="$c" ;;
      five_hour_pct) color_5h="$c" ;;
      seven_day_pct) color_7d="$c" ;;
    esac
  done

  # Format resets_at as remaining time
  format_resets_remaining() {
    ...
    if [ "$remaining" -le 0 ]; then
      ...
      format_elapsed_time "$remaining"
    fi
  }

  # Build individual value with optional reset time (shown when ≥50%)
  build_pct_value() {
    local pct="$1" color="$2" resets_epoch="$3"
    [ -z "$pct" ] && return
    pct=$(printf "%.0f" "$pct")
    local text="${pct}%"
    if [ "$pct" -ge 50 ] && [ -n "$resets_epoch" ]; then
      local reset_str
      reset_str=$(format_resets_remaining "$resets_epoch")
      text="${text}~${reset_str}"
    fi
    printf "%b%s%b" "$color" "$text" "$THEME_RESET"
  }

  local slash
  slash=$(printf "%b/%b" "$COLOR_DARK_GRAY" "$THEME_RESET")

  local v_ctx v_5h v_7d
  v_ctx=$(build_pct_value "$ctx_pct" "$color_ctx" "")
  v_5h=$(build_pct_value "$five_hour_pct" "$color_5h" "$five_hour_resets")
  v_7d=$(build_pct_value "$seven_day_pct" "$color_7d" "$seven_day_resets")

  # Join available values with "/"
  local inner=""
  for v in "$v_ctx" "$v_5h" "$v_7d"; do
    [ -z "$v" ] && continue
    inner="${inner:+${inner}${slash}}${v}"
  done

  [ -n "$inner" ] && printf "%b(%b%s%b)%b" "$COLOR_DARK_GRAY" "$THEME_RESET" "$inner" "$COLOR_DARK_GRAY" "$THEME_RESET"
}
