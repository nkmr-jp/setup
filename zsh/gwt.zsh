...
_gwt_yolo() {
    local original_dir="$(pwd)"
    _gwt_quick $1 && claude --permission-mode auto && cd ${original_dir} && _iterm2_send_current_dir 2>/dev/null
}
...
${YELLOW}コマンド:${RESET}
  ...
  yolo, y <prefix> [base]    日付付きworktreeを作成してClaude Code (auto mode) を起動
  help, h                    このヘルプを表示
...
${YELLOW}使用例:${RESET}
  ...
  gwt yolo feature/login develop    # 日付付きworktreeを作成してClaude Code (auto mode) を起動
...
_gwt_completion() {
    ...
    commands=(
        ...
        'yolo:worktreeを作成してClaude Code (auto mode) を起動'
        'help:ヘルプを表示'
    )
