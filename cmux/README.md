...
- SessionEnd でエントリを削除。SessionEnd を逃した分（クラッシュ・workspace 強制クローズ等）は書き込み時に prune する（setup#5）: 生存 surface UUID 集合（workspace.list + surface.list、60 秒スロットル）に無い surfaceId のエントリを削除し、RPC 失敗時はフォールバックの 7 日超 prune のみ。ファイルには現在アクティブな cmux セッションだけが残る。
